...
void printGridWithColor(){
	map<char,index> colorTable;
	int colorNum = 0;
	for(char ch='a';ch<='z';++ch,++colorNum)
		colorTable[ch] = {15,(colorNum % 15)+5};
	...
	for(vc row:grid){
		for(char element:row){
			if(element == '#')
				...
				index colorValues = colorTable[element];
				color(colorValues.first,colorValues.second);
				cout<<element<<" ";
				color();
			}
		}
...
//	printGridWithColor();
	...
	grid[defect.first][defect.second] = '#';
